...
const FirstNav = ({ scrollTop, categories, socialLonks, navLinks, isMenuOpen, setIsMenuOpen }: FirstNavProps) => {
  return (
    <ContainerX className='py-7 flex flex-col lg:flex-row justify-between lg:items-center gap-6 lg:gap-3 xl:gap-6'>
        <div className='flex flex-col md:flex-row md:items-center gap-12 md:gap-6 lg:gap-3 xl:gap-6 justify-between'>
            ...
            <MobileMenu isMenuOpen={isMenuOpen} setIsMenuOpen={setIsMenuOpen} scrollTop={scrollTop} />

                <TransitionLink href='/' className='flex gap-2 md:gap-1.5 items-center cursor-pointer'>
            <CardImage src="/assets/icons/logo.svg" alt="Luminae Logo" className='w-10 h-6.5 md:w-12.5 md:h-8.5' />
            <Heading level={1} size='h32' weight='bold' lineH='lh20'>Luminae</Heading>
            </TransitionLink>
            
            <CartBtn />
            </div>
        <div className='flex items-center justify-between gap-1 border border-Grey-200 rounded-sm px-3 py-2 w-full md:max-w-86.5'>
                            <input type="text" placeholder='Search Products' className='flex-1 min-w-fit border-none outline-none focus:outline-none text-sm font-normal leading-[20px] placeholder:text-Grey-400' />
            <div className='flex items-center gap-3 w-fit'>
               
                    <Dropdown className='pr-3 border-r border-Grey-100' size='w-6 h-6' data={categories} />
        ...
        <div className='w-1/2 lg:w-full flex items-center justify-between lg:justify-around gap-1'>
            {
                navLinks.map((link, index) => (
                    <TransitionLink key={index} href={link.href} className='text-Grey-800 text-sm font-notmal leading-[20px]'>{link.name}</TransitionLink>
                ))
            }
        </div>
        <div className='flex gap-3 items-center'>
            {
                socialLonks.map((link, index) => (
                    <TransitionLink key={index} href={link.link} className='text-Grey-300' onClick={scrollTop}>
                        {link.icon}
                    </TransitionLink>
                ))
            }
